...
try:
	if len(posts) < args.unique:
		raise Exception(f"Not enough tags in search to fill the unique count. Stopping. {len(posts)} / {args.unique}")
	...
	if downloads_this_run == args.unique or args.unique == iters - 1 or iters - 1 == len(posts):
		print(f"Done downloading specified number of unique images.")
		print(f"D:{downloads_this_run}, S:{iters-downloads_this_run}/{len(posts)}, I:{iters-1}/{len(posts)}")
